/* 
@Description: Lookup component for equipment request screen. core has been implemented by salesforce Team. made edit it part of equipment request.
Feaature: Multiselect, Add new record and edit existing record. 
*/
...
export default class LwcDdMxLookup extends LightningElement {
   // Public properties
   ...
   get displayEditButton() {
      return this.searchObject === 'Contact';
   }
   @api
   callApexMethood() {
      var conResults = [];
      search({
         ...
         .then((conResult) => {
            this._defaultSearchResults = [...conResult];
            this.setSearchResults(conResult);
   ...
   renderedCallback() {
      if (this.hasRendered === false && this.searchObject === 'Contact') {
         this.callApexMethood();
   ...
   setSearchResults(results) {
      ...
      // Reset the spinner
      //this.newSearchResults = [...this._defaultSearchResults];
      this.loading = false;
   ...
   setDefaultResults(results) {
      console.log('calling set default results ' + JSON.stringify(results));
   ...
   connectedCallback() {
      ...
      const searchEvent = new CustomEvent('load', {
         detail: {
            searchTerm: '',
            rawSearchTerm: '',
            searchTermObject: this.searchObject,
   ...
   processSelectionUpdate(isUserInteraction) {
      ...
      if (isUserInteraction) {
         const selectedIds = this._curSelection.map((sel) => sel.id);
         this.tabletSelection = this._curSelection;
         this.dispatchEvent(
            new CustomEvent('selectionchange', {
               detail: this._curSelection,
               searchTermObject: this.searchObject,

               // curSelection: this._curSelection
            }),
         );
      }
   }
   ...
   handleEditClick(event) {
      const editContactEvent = new CustomEvent('editcontactrecord', {
         detail: event.target.dataset.key,
         contactId: event.target.dataset.key,
      });
      this.dispatchEvent(editContactEvent);
   }
   ...
   handleNewRecordClick(event) {
      ...
      const newRecordEvent = new CustomEvent('newrecord', {
