...
import newSession from "@salesforce/apex/ServiceConsoleCaseTimerController.newSession";
import totalTime from "@salesforce/apex/ServiceConsoleCaseTimerController.totalTime";
import getSessions from "@salesforce/apex/ServiceConsoleCaseTimerController.getSessions";
import newSessionManual from "@salesforce/apex/ServiceConsoleCaseTimerController.newSessionManual";
import checkAccess from "@salesforce/apex/ServiceConsoleCaseTimerController.checkAccess";
...
const OMNI_OFFINE_ANOTHER_BROWSER = 'Offline-Another Browser';
const OMNI_OFFLINE_CONNECTION_ERROR =  'Offline-Connection Error';
...
export default class ServiceConsoleCaseTimer extends LightningElement {
    ...
    *****************************************************************************************************************************/
    ...
    set caseStatus(value) {
        ...
        if (this._caseStatus && this._caseStatus !== value) {
            ...
            newSession({caseId: this.recordId, timeVal: this.totalMilliseconds, status: this.caseStatus, omniStatus: this.omniStatus, newCaseStatus: value, newOmniStatus: BLANK_STRING}).then(() => {
    ...
    @api
    get omniStatus() {
        return this._omniStatus;
    }
    set omniStatus(value) {
        this.logToConsole("omniStatus: " + value);
        // Before updating the status create a new time entry and use the previous status value
        if (this._omniStatus && this._omniStatus !== value) {
            this.timeSaved = true; // Ensures we only save once as this event can be called multiple times
            this.logToConsole("omniStatus: changed");
            this.stop();
            this.logToConsole("Saving new session " + this.totalMilliseconds);
            newSession({caseId: this.recordId, timeVal: this.totalMilliseconds, status: this.caseStatus, omniStatus: this.omniStatus, newCaseStatus: BLANK_STRING, newOmniStatus: value}).then(() => {
                    // Reload the values from the DB so we have the latest
                    refreshApex(this.sessions);
                    refreshApex(this.total);
                    this.totalMilliseconds = 0;
                    ...
                    this.timerStartTime = Date.now();
                    this.pausedStartTime = this.timerStartTime;
                    if (this.manualPause || value === OMNI_OFFINE_ANOTHER_BROWSER || value === OMNI_OFFLINE_CONNECTION_ERROR) {
                        this.updateTime();
                    }  else {
                        this.start();
                    }
    ...
    *****************************************************************************************************************************/
    ...
    //Modal
    modalClosed = true;
    modalClass = CLASS_SLDS_HIDE;
    manualDate;
    manualDuration = DEFAULT_TIMER;
    comments;
    ...
    *****************************************************************************************************************************/
    ...
    @wire(getSessions, {recordId: '$recordId'}) sessions;
    ...
    @wire(totalTime, {recordId: '$recordId'}) wireTotalTime(result) {
        ...
        if (result.data) {
            ...
            this.formattedTime = DEFAULT_TIMER;
        } else {
    ...
    * Event Handlers
    *
    * Note that ES6 function declaration pattern functionName = () => {} allows for binding the function properly to 'this'
    *
    *****************************************************************************************************************************/
    ...
    disconnectedHandler = () => {
        ...
        if(!this.timeSaved && this.stime !== DEFAULT_TIMER && this.totalMilliseconds > (this.bufferInSeconds*1000)){
            this.timeSaved = true; // Ensures we only save once as this event can be called multiple times
            this.stop();
            this.logToConsole("Saving new session " + this.totalMilliseconds);
            ...
            newSession({caseId: this.recordId, timeVal: this.totalMilliseconds, status: this.caseStatus, omniStatus: this.omniStatus, newCaseStatus: '', newOmniStatus: '' }).then(() => {
                refreshApex(this.sessions);
                refreshApex(this.total);
            })
            .catch(error => {
    ...
    pauseTimer = (pause) => {
        switch(pause){
            ...
            case false:
                ...
                if(this.stime !== DEFAULT_TIMER && !this.manualPause && !this.playing){
                    this.start();
                }
                break;
    ...
    toggleModal() {
        ...
        this.modalClass = this.modalClosed ? CLASS_SLDS_HIDE : BLANK_STRING;
    }

    //clear manual session creation form inputs
    clearInputs() {
        this.manualDate = null;
        this.manualDuration = DEFAULT_TIMER;
        this.comments = null;
    }
    ...
    handleSaveSession(){
        ...
        if (isValid) {
            newSessionManual({caseId: this.recordId, timeVal: this.manualDuration, theDate: new Date(this.manualDate), comments: this.comments, status: this.caseStatus, omniStatus: this.omniStatus})
            .then(result => {
                if (result) {
                    this.dispatchEvent(
                        new ShowToastEvent({
